...
class BuildExtensionPlugin {
    ...
    async build() {
        ...
        for (const browser of ['chrome', 'firefox']) {
            ...
            try {
                await this.execute(
                    `cd zip/${browser} && zip ../${browser}.zip * -r && cd ../..`
                )
            } catch {
                console.warn(
                    'Please install the zip util to get automatic zipping'
                )
                // No zip installed
            }
        ...
        if (this.production) {
            await this.execute(
                'git archive --format zip --output zip/InstagramDownloader.zip HEAD'
            )
        }
    }
